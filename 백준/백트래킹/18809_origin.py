...
visit_possible = [False] * len(possible)

lands = [''] * (g+r)
def back_for_land(x,s):
    if x == (g+r):
        back_for_seed(lands)
        return
    
    for i in range(s,len(possible)):
        a,b = possible[i]
        lands[x] = (a,b)
        back_for_land(x+1,i+1)


def back_for_seed(lands):
    red_seeds = combinations(lands, r)
